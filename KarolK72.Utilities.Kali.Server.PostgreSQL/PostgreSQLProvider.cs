...
namespace KarolK72.Utilities.Kali.Server.PostgreSQL
{
    public class PostgreSQLProvider : ISqlProvider
    {
        ...
        private const string InsertLogSQL = @"  INSERT INTO KaliLogs(
                                                    LogLevel, Category, EventID, EventName, Scopes, ExceptionJSON, DateTimeCreated, DateTimeModified, RowVer
                                                    )
                                                VALUES(
	                                                @LogLevel, @Category, @EventID, @EventName, @Scopes, @ExceptionJSON, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, @RowVer
                                                ) RETURNING *";
